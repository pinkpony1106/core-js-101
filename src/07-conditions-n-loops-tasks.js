...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    let numberOfCard = Number(array[i]);
...
function getDigitalRoot(num) {
  ...
  const newArray = array.map((element) => Number(element));
...
function toNaryString(num, n) {
  const x = num % n;
  return (num >= n ? toNaryString(Math.floor(num / n), n) : '') + x;
}
...
function getCommonDirectoryPath(pathes) {
  const array = pathes.map((element) => element.split('/'));
